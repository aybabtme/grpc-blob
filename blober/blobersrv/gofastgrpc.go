...
func (b *GoFastGRPCBlober) Write(srv service.Blober_WriteServer) error {
	ctx := srv.Context()
	req, err := srv.Recv()
	if err == io.EOF {
		return nil
...
func (b *GoFastGRPCBlober) Read(req *service.ReadReq, srv service.Blober_ReadServer) error {
	ctx := srv.Context()
	if req.GetBufSize() < 1 {
